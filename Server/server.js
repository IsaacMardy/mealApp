...
app.get('/getMeal', async (req,res)=>{
    try{
        const db = client.db('Mealdatabase');
        const collection = db.collection('Meals')
        const meal = await collection.findOne({ _id: req.body.userID });
...
app.get('/getSchedule', async (req, res)=>{
    try{
        ...
        const schedule = await collection.findOne({ userID: req.body.userID });
        if (schedule) {
            return  res.status(200).json({ success: true, schedule: schedule})
        }else{
            return  new Schedule(req.body.userID);
        }
    }catch(error){
        res.status(500).send(error)
    }
...
app.post('/saveSchedule', async (req, res)=>{
    try {
        const db = client.db('Mealdatabase');
        const collection = db.collection('Schedules');
        const exist = await collection.findOne({ userID: req.body.userID });
        if (exist) {
            await collection.replaceOne({ userID: req.body.userID }, req.body);
        }else{
            await collection.insertOne(req.body)
        }
